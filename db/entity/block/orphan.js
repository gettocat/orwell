...
orphan.prototype.add = function (blockjson) {
    if (config.debug.blockchain.orphanblock)
        console.log("add block to orphan: " + blockjson.hash)
...
orphan.prototype.checkBlock = function (block) {
    ...
    do {
        ...
        if (config.debug.blockchain.orphanblock)
            console.log("find prev in orphan " + b, !!block.hash)
        if (block.hash)
            arr.push(block);
    } while (block.hash);
    ...
    if (config.debug.blockchain.orphanblock)
        console.log("find prev in mainchain " + arr[arr.length - 1].prev_block, !!_block.hash)
    if (_block && _block.hash) {
        if (config.debug.blockchain.orphanblock)
            console.log("have block " + arr[arr.length - 1].prev_block + " in main chain")
        var height = indexes.get("block/" + _block.hash).height;
        if (height + arr.length > indexes.get('top').height) {
            if (config.debug.blockchain.orphanblock)
                console.log("height of block in mainchain " + height + ", common length " + (height + arr.length) + ",  top height: " + indexes.get('top').height + ", merge main chain!");
            //replace mainchain with this chain
            if (config.debug.blockchain.orphanblock)
                console.log("*** START MERGE BRANCH: ***")
            ...
            for (var i = arr.length - 1; i >= 0; i--, startheight++) {
                ...
                if (typeof binfo == 'string') {
                    if (config.debug.blockchain.orphanblock)
                        console.log("find hash in mainchain ", binfo, "height", startheight)
                    var b = blockchain.getBlock(binfo);
                    blockchain.replaceBlock(startheight, b.hash, arr[i]);
                    if (config.debug.blockchain.orphanblock)
                        console.log("replace blocks", b.hash, "->", arr[i].hash);
                    this.del(arr[i].hash);
                    if (config.debug.blockchain.orphanblock)
                        console.log("remove From Orphan", arr[i].hash);
                    this.add(b);
                } else {
                    if (config.debug.blockchain.orphanblock)
                        console.log("not finded hash in mainchain ", binfo, "height", startheight, "add as new")
                    var f = this;
                    blockchain.appendBlockFromJSON(arr[i], function (block, isExist, iMainChain) {
                        if (config.debug.blockchain.orphanblock)
                            console.log("remove From Orphan", block.hash);
                        f.del(block.hash);
                    });
                }
            ...
            if (config.debug.blockchain.orphanblock)
                console.log("*** STOP MERGE BRANCH: ***")
        } else {
            if (config.debug.blockchain.orphanblock)
                console.log("height of block in mainchain " + height + ", common length " + (height + arr.length) + ",  top height: " + indexes.get('top').height + ", dont merge main chain!");
            if (indexes.get('top').height - height + arr.length > 100) {//do not save orphan block more that 100 height over
                if (config.debug.blockchain.orphanblock)
                    console.log("height-range of blocks is more than 100, remove from orphan");
                for (var i in arr) {
                    this.del(arr[i].hash);
                }
            }
        ...
        if (config.debug.blockchain.orphanblock)
            console.log("havent block " + arr[arr.length - 1].prev_block + " in main chain, send getblock")
...
orphan.prototype.check = function (hash) {
    ...
    if (config.debug.blockchain.orphanblock)
        console.log("get block " + hash, block.prev_block, !!block.hash)
    if (!block || !block.hash)
        return false;
