...
builder.prototype.sign = function (private_keys) {
    ...
    for (var i in this.inputs) {
        ...
        var tx = this.rawunsigned + "01000000",
                ...
                sig = cr.sign(new Buffer(private_keys[i], 'hex'), sig256),
                scriptSig = script.scriptSig(sig, new Buffer(cr.getPublicByPrivate(private_keys[i]), 'hex'));

        this.scriptSigRaw[i] = [
            sig.toString('hex'),
            cr.getPublicByPrivate(private_keys[i])
        ];
...
builder.prototype.verify = function () {
    ...
    for (var i in this.inputs) {
        ...
        res[i] = cr.verify(pubkey, sign, hash2sign);
    }
