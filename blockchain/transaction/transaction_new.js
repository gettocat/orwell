...
transaction.prototype = {
    ...
    send: function () {
        ...
        var nodeNames = protocol.getNodeList();
        ...
        for (var i in nodeNames) {
            var rinfo = protocol.getUniqAddress(nodeNames[i]);
            var nodeInfo = nodes.get("data/" + nodeNames[i]);
            var filter = nodeInfo.filter;
            if (filter) {//spv client, send only header
                var inventory_filter = new inv('newtx', this.getHash(), this.toJSON(), filter);
                var list = inventory_filter.getList();
                if (list.object_list.length > 0)
                    protocol.sendOne(rinfo, 'inv', list);
            } else {
                protocol.sendOne(rinfo, 'inv', obj);
            }
    ...
    reject: function (rinfo) {
        var protocol = require('../../network/protocol')
        protocol.sendOne(rinfo, 'reject', {
            hash: this.getHash(),
            errors: this.errors
        });
...
transaction.checkTxFilter = function (txJson, filter) {
    if (filter.contains(new Buffer(txJson.hash, 'hex'))) {
        return true;
    } else {
        for (var k in txJson.in) {
            if (!txJson.coinbase) {
                var h = Script.sigToArray(txJson.in[k].scriptSig)
                var publicKey = h[1];
                if (filter.contains(new Buffer(publicKey, 'hex')))
                    return true;

                if (filter.contains(new Buffer(hash.generateAddressHash(publicKey), 'hex')))
                    return true;
            } else {
                if (filter.contains(new Buffer(txJson.in[k].scriptSig, 'hex')))
                    return true;
            }

            if (filter.contains(new Buffer(txJson.in[k].hash, 'hex')))
                return true;
        }

        for (var o in txJson.out) {
            var hash160 = Script.scriptToAddrHash(txJson.out[o].scriptPubKey);//its buffer
            if (filter.contains(hash160))
                return true;
            if (filter.contains(txJson.out[o].scriptPubKey))
                return true;
