...
function difficulty(bits) {
    var b = new BN(bits2target(config.blockchain.mining.maxtarget), 16);
...
function checkBits(block) {
    ...
    if (block.height <= 199)
        return config.blockchain.mining.maxtarget;
...
function getBitsRange(nActualTimespan, block) {
    ...
    var limit = bits2targetBN(config.blockchain.mining.maxtarget);
...
function getBlockValue(fee, height) {
    ...
    if (height == 0)
        return 50 * config.blockchain.satoshicoin + fee;

    if (height <= 190)
        return new BN(height * 100 * config.blockchain.satoshicoin).add(new BN(fee)).toString(10);

    if (height >= 191 && height <= 199) {
        return new BN(13500 * config.blockchain.satoshicoin).add(new BN(fee)).toString(10);//1 936 000
    }

    if (height == 200) {
        return new BN(15000 * config.blockchain.satoshicoin).add(new BN(fee)).toString(10);//1 951 000
    }

    //this period need me to create some infrastructure and finish some projects.
    if (height > 200 && height <= 10000) {
        return new BN(5 * config.blockchain.satoshicoin).add(new BN(fee)).toString(10);//49 000
    }
    ...
    return new BN(reward).mul(new BN(config.blockchain.satoshicoin)).add(new BN(fee)).toString(10);
}
