...
var txTool = require("../transaction/tools")
var merkle = require('../merkle/tree');
var util = require('../../tools/util');
var hash = require('../../crypto/hash');
var blockvalidator = require('../../blockchain/block/validator')
...
block.prototype = {
    ...
    fromJSON: function (json) {
        ...
        for (var i in json.tx) {
            ...
            var t = txTool.createFromJSON(json.tx[i])
            this.vtx.push(t);
    ...
    send: function () {
        ...
        var inventory = new inv('newblock', this.getHash(), this.toJSON());
        ...
        for (var i in nodeNames) {
            ...
            if (filter) {//spv client, send only header
                protocol.sendOne(rinfo, 'inv', {
                    synced: false,
                    object_type: 'block',
                    object_count: 1,
                    object_offset: 0,
                    object_next_offset: 0,
                    object_listed: 1,
                    object_list: [
                        this.getHeaderHex()
                    ],
                    object_queryhash: null,
                });
            } else {
