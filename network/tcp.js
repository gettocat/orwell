...
module.exports = function () {
    ...
    try {
        ...
        netevent.emit("net.server.error", e)
    }
    ...
    netevent.on("net.connection.add", function (socket, from) {
        ...
        if (config.debug.peers)
            console.log("add seed " + addr, from);
    ...
    netevent.on("net.connection.remove", function (addr, from) {
        ...
        if (config.debug.peers)
            console.log("remove seed " + addr, from);
    ...
    netevent.on("net.message", function (socket, data, from) {
        ...
        if (config.debug.network)
            console.log("<< " + protocol.getAddressUniq(socket) + " > " + (new Buffer(data).length / 1024), "Kb\n");
    ...
    netevent.on("net.error", function (e) {
        ...
        if (e.code === 'ECONNRESET') {
            ...
            //nodes.set("connection/" + e.removeAddress + ":" + e.port, null);
            showed = 1;
        }
    ...
    netevent.on("net.server.init", function (server) {
    ...
    function addNode(addr, cb) {
        ...
        if (!socket.STATUS || socket.destroyed === true) {
            ...
            var client = p2p.newClient(a.remoteAddress, a.port);
    ...
    function send(addr, msg) {
        var sendmessage = function (msg, socket) {
            socket.write(msg, function () {
                if (config.debug.network)
                    console.log(">> " + protocol.getAddressUniq(socket) + " > (" + msg.length / 1024 + "K bytes)\n");
            });
        }
